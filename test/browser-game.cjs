...
(async()=>{
 ...
 await pages[0].locator('#start-game-btn').click();await Promise.all(pages.map(p=>p.locator('#game-screen.active').waitFor()));
 await Promise.all(pages.map(p=>p.waitForFunction(()=>typeof state!=='undefined' && state.revision>0)));
 let turns=0;
 while(turns++<600){
  ...
  await Promise.all(pages.map(p=>p.waitForFunction(rev=>typeof state!=='undefined' && (state.revision>rev||document.querySelector('#end-screen.active')),s.revision)));
 }
